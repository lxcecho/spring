...
import com.lxcecho.exception.InvalidUserException;
import com.lxcecho.service.HelloService;
...
public class HelloController {
	...
	public String sayHi(Model model) {
		String mvc = helloService.say("MVC ") + LocalDate.now();
		model.addAttribute("lxcecho", mvc);
		return "hello";
	}
	 ...
	 *
	 * @param name
	 * @return ModelAndView、Model、void\String xxxx  请求数据到自定义 Object 属性的映射需要用到（WebDataBinder【数据类型转换、绑定错误处理】）Factory
	 */
//	@ResponseBody
	@GetMapping("/hello") // 所有的 xxxMapping 都是 RequestMapping
	public String sayHello(String name, // 可以从请求参数中得到
						   @RequestParam("user") String user, // 可以从请求参数中得到
						   HttpSession session,/* HttpServletRequest request,*/ // 原生的 session 对象
